...
res_table_unshrunken <- results(PDdds, contrast=contrast_, alpha = 0.05, lfcThreshold = 0.6)
...
res_table_unshrunken <- res_table_unshrunken %>%
  ...
  as_tibble()
write.csv(res_table_unshrunken, file="results/PD_VS._control_res_table_unshrunke.csv")
...
write.csv(sigOE, file="results/PD_VS._control_DEGslfc0.6.csv")
```
...
```{r}
write.csv(df.out, file="results/PD_VS._control_ConvertedDEGslfc0.6.csv")
...
res_table_shrunken_2 <- res_table_shrunken %>% 
                  mutate(threshold_OE = abs(log2FoldChange) > 0.6)
...
jpeg(file="results/PD_VS._Control_DEGsHeatMaplfc0.6.png")
